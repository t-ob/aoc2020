use std::{collections::{HashMap, HashSet}, io::{self, Read}, vec};

use rand::seq::SliceRandom;
use rand::Rng; // 0.7.2

use day20::{D4, Picture, Tile10, Tile8};
...
fn main() -> io::Result<()> {
        ...
        .zip([&mut neighbours_to_left, &mut neighbours_to_up].iter_mut())
    {
        symmetries
            ...
            .for_each(|(signature, xy)| {
                let neighbours = neighbour_map.entry(signature).or_insert(HashSet::new());
    ...
    println!("{}", picture.symmetries().iter().map(|sym| sym.choppiness()).min().unwrap());
